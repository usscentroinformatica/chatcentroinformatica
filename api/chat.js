...
function App() {
  ...
  // Función para manejar click en botones (envía el value como mensaje)
  const handleOptionClick = async (optionValue) => {
    console.log('Enviando valor de botón:', optionValue);  // DEBUG: Ver en consola
  ...
  const handleSendMessage = async (msg = input) => {
    ...
    try {
      ...
      console.log('Respuesta del backend:', data);  // DEBUG: Ver si options llega

      if (data.response) {
        const botMessage = { 
          type: 'bot', 
          text: data.response,
          options: data.options || null  // Guarda opciones para renderizar
        };
  ...
  // Splash Screen Component (mismo que antes)
  if (showSplash) {
    return (
          ...
          <div key={index} className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            {message.type === 'bot' && (
              <div className="flex flex-col items-start gap-3 max-w-[85%] md:max-w-[70%] group">
                ...
                {/* Renderiza botones si hay options */}
